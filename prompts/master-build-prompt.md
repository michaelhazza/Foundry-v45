...
## Version Reference
- **This Document**: master-build-prompt.md v41
- **Linked Documents**:
...
└── docs/
    ├── 02-ARCHITECTURE.md          ← Agent 2 (optional, for ADRs/config guidance)
...
- `docs/02-ARCHITECTURE.md` - ADRs and config templates. Claude Code MAY read this for implementation guidance but MUST NOT treat it as a required input.
...
Phase 0: Extract Gate Scripts (Step 0 - MANDATORY FIRST STEP)
  ...
  |-- Verify: ls scripts/*.sh | wc -l (should equal 26)
  |-- Creates scripts/run-all-gates.sh + 25 verify-*.sh scripts
  |
  v
...
Phase 3: Gate Execution
  ...
  |-- If ANY gate fails -> fix and re-run automatically
  |-- Produces docs/build-gate-results.json
  |
  v
Phase 4: Build Proof
...
After all gates pass and build proof is generated, move specification prompts out of the root directory to reduce clutter.

**Trigger:** All gates PASSED, `docs/build-gate-results.json` exists with `summary.failed === 0`
...
FAILED=$(jq -r '.summary.failed' docs/build-gate-results.json)
if [ "$FAILED" != "0" ]; then
  echo "[SKIP] Build has failures - skipping file organisation"
  exit 0
fi
...
### Total Pre-Implementation Artifacts: 9 (5 JSON + 2 MD + 1 JSON template + 1 shell)
...
6. Any gate returns FAIL (via run-all-gates.sh ONLY)
7. build-gate-results.json shows summary.failed > 0
...
- [OK] 9 pre-implementation artifacts (5 JSON + 4 Agent 6 outputs)
